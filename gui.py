...
def show_splash():
    ...
    # Outer glow border
    border = tk.Frame(splash, bg=ACCENT_BLUE, bd=2)
    border.place(relx=0, rely=0, relwidth=1, relheight=1)
    inner = tk.Frame(border, bg=BG_BASE)
    inner.place(relx=0.004, rely=0.006, relwidth=0.992, relheight=0.988)

    # Logo
    # To use a custom logo.png image later, uncomment the following block:
    # try:
    #     logo_img = tk.PhotoImage(file="logo.png")
    #     logo_lbl = tk.Label(inner, image=logo_img, bg=BG_BASE)
    #     logo_lbl.image = logo_img
    #     logo_lbl.pack(pady=(28, 0))
    # except Exception:
    #     tk.Label(inner, text="◆", bg=BG_BASE, fg=ACCENT_BLUE, font=("Segoe UI", 36)).pack(pady=(28, 0))
    
    tk.Label(inner, text="◆", bg=BG_BASE, fg=ACCENT_BLUE,
             font=("Segoe UI", 36)).pack(pady=(28, 0))
    tk.Label(inner, text="NetPhantom", bg=BG_BASE, fg=TEXT_PRIMARY,
             font=("Segoe UI", 22, "bold")).pack(pady=(0, 2))
    tk.Label(inner, text="Professional Network Packet Analyzer  v3.0",
             bg=BG_BASE, fg=ACCENT_CYAN,
    ...
    # Progress area
    prog_frame = tk.Frame(inner, bg=BG_BASE)
    prog_frame.pack(pady=(22, 4))
    status_lbl = tk.Label(prog_frame, text="Loading capture engine...",
                          bg=BG_BASE, fg=TEXT_DIM, font=("Segoe UI", 8))
    status_lbl.pack()
    ...
    style.configure("Splash.Horizontal.TProgressbar",
                    ...
                    bordercolor=BG_BASE, lightcolor=ACCENT_BLUE,
                    darkcolor=ACCENT_BLUE)
    bar = ttk.Progressbar(prog_frame, length=380, mode="determinate",
                          style="Splash.Horizontal.TProgressbar")
    bar.pack(pady=8)

    messages = [
        (20, "Initializing Scapy engine..."),
        (40, "Detecting network interfaces..."),
        (60, "Loading protocol analyzers..."),
        (80, "Building GUI components..."),
        (100, "Ready!"),
    ]
    ...
    def advance(idx=0):
        if idx < len(messages):
            ...
            splash.after(280, advance, idx + 1)
        else:
            splash.after(400, splash.destroy)

    splash.after(200, advance)
    splash.mainloop()
...
class PacketSnifferGUI:
    ...
    def _build_toolbar(self):
        ...
        self._filter_entry = tk.Entry(toolbar, width=18, bg=BG_INPUT, fg=ACCENT_GREEN,
                                       insertbackground=ACCENT_GREEN, font=FONT_MONO_SM,
                                       relief="flat", bd=4)
        self._filter_entry.insert(0, self._config["default_filter"])
        self._filter_entry.pack(side=tk.LEFT, padx=(0, 8), pady=8)
        ...
        self._proto_combo = ttk.Combobox(
            ...
            width=8, font=FONT_MONO_SM, style="Custom.TCombobox")
        self._proto_combo.pack(side=tk.RIGHT, padx=(4, 12), pady=8)
        tk.Label(toolbar, text="Protocol:", bg=BG_HEADER, fg=TEXT_SECONDARY,
    ...
    def _draw_bar_chart(self, protos: dict, total: int):
        ...
        if not items:
            return
        bw = max(14, (w - 20) // max(len(items), 1) - 6)
        x, mh = 10, 80
        for proto, count in items:
            ...
            c.create_rectangle(x, 90, x + bw, 90 - bh, fill=col, outline="", width=0)
            # Glow effect (slightly transparent overlay at top)
            c.create_rectangle(x, 90 - bh, x + bw, 90 - bh + 2, fill=col, outline="")
            # Label
            c.create_text(x + bw // 2, 100, text=proto[:4], fill=col,
                         font=("Segoe UI", 7))
            c.create_text(x + bw // 2, 90 - bh - 10, text=str(count), fill=col,
    ...
    def _update_graph(self, stats: dict):
        ...
        for i in range(5):
            ...
            c.create_line(pad_left, y, w - pad_right, y, fill=BORDER, dash=(2, 4))
        ...
        if len(points) >= 2:
            ...
            c.create_line(flat_line, fill=ACCENT_BLUE, width=2, smooth=True)

            # Current value dot
            if points:
                lx, ly = points[-1]
                c.create_oval(lx - 3, ly - 3, lx + 3, ly + 3, fill=ACCENT_CYAN, outline="")
                c.create_text(lx, ly - 12, text=f"{history[-1][1]}", fill=ACCENT_CYAN,
                             font=("Segoe UI", 8, "bold"))
    ...
    def _make_btn(self, parent, text: str, color: str, cmd) -> tk.Button:
        ...
        btn.bind("<Leave>", lambda e, b=btn: b.config(bg=BG_INPUT))
        return btn
...
#  Launch
# ──────────────────────────────────────────────
def run_gui(open_file: str = None):
    ...
    # To set a custom window icon later, uncomment the following block:
    # try:
