...
# ─────────────────────────────────────────────
def build_protocol_tree(pkt, timestamp: str = None) -> list:
    """
    ...
    Args:
        ...
        timestamp: Optional capture timestamp string (HH:MM:SS.mmm). When omitted,
                   falls back to the current time which reflects the click time, not
                   the actual packet capture time.
    """
    tree = []

    # Frame info
    cap_time = timestamp or datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
    frame_fields = [
        ("Frame Length", f"{len(pkt)} bytes"),
        ("Capture Time", cap_time),
    ]
    ...
    if pkt.haslayer(DNS):
        ...
        if dns.qd and pkt.haslayer(DNSQR):
            try:
                ...
                pass
        # Answer section — DNSRR is a Scapy linked-list, not a random-access list;
        # traverse via the .payload chain instead of dns.an[i] indexing.
...
class PacketAnalyzer:
    ...
    def _detect_threats(self, info: dict, now: float) -> str | None:
        ...
        if now - state["window_start"] >= 1.0:
            ...
            state["ssh_counter"].clear()
            state["rdp_counter"].clear()
            state["port_targets"].clear()
        ...
        # Cleartext Credential Detection (HTTP Basic Auth / POST form credentials / FTP)
        payload = info.get("payload_ascii", "").lower()
        if info["protocol"] in ("HTTP", "FTP", "TELNET"):
